...
import com.room_services.repository.SkippedRoomDocumentRepository;
		...
		try (Workbook workbook = new XSSFWorkbook(inputStream)) {
			...
			for (int i = 0; i <= sheet.getLastRowNum(); i++) {
				...
				if (row == null) {
					...
					reasons.put(displayRow, "Empty_Row");
					skippedRoomDocuments
							.add(buildSkippedRoomDocument(displayRow, Collections.EMPTY_MAP, "Empty_Row", batchId));
					continue;
				}
				...
				if (!StringUtils.hasText(name)) {
					...
					reason = "Missing or valid room price";
				} else if (floor == null) {
				...
				Room room = new Room();
				room.setName(name);
				log.debug("Prepare to save room: {}", room);
				validRoom.add(room);
			}
			log.info("Valid room to save: {}", validRoom.size());

			return skippedRoomDocumentRepository.saveAll(skippedRoomDocuments)
					.thenMany(roomRepository.saveAll(validRoom).doOnNext(r -> log.info("save room: {}", r))
							.doOnError(err -> log.error("Error saving room: {}", err.getMessage(), err)))
					.collectList().map(saved -> new RoomImportSummary(saved.size(), skippedRoomDocuments.size(),
							skippedRow, reasons));
