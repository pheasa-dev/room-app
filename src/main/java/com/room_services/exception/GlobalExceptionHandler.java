...
public class GlobalExceptionHandler {
	...
	public Mono<ProblemDetail> handlRoomNotFound(RoomNotFoundException ex, ServerWebExchange exchange) {
		...
		return Mono.just(problemFactory.create(HttpStatus.NOT_FOUND, ex.getMessage(), ErrorCode.ROOM_NOT_FOUND.name(),
				exchange));
	}
	...
	public Mono<ProblemDetail> handleConstraintViolation(WebExchangeBindException ex, ServerWebExchange exchange) {
		...
		return Mono.just(problemFactory.create(HttpStatus.BAD_REQUEST, ex.getMessage(),
				ErrorCode.CONSTRAINT_VIOLATION.name(), exchange));
	}
	...
	public Mono<ProblemDetail> handleGeneric(Exception ex, ServerWebExchange exchange) {
		...
		return Mono.just(problemFactory.create(HttpStatus.INTERNAL_SERVER_ERROR, "Unexpected Error: " + ex.getMessage(),
				ErrorCode.SYSTEM_ERROR.name(), exchange));
	}
}
